// ベン図の各領域の文字表示位置を正しく計算（領域の内側に配置）
// 2円のみの重なり = 重心と遠い交点の間（領域の内側）
...
console.log('=== ベン図の文字表示位置の正しい計算（領域の内側）===\n');
...
console.log('重心から遠い交点（2円のみの重なり領域の外縁）:');
...
// 2つの交点間の弦の長さを計算
const chordLength12 = Math.sqrt((int12.i1.x - int12.i2.x) ** 2 + (int12.i1.y - int12.i2.y) ** 2);
const chordLength13 = Math.sqrt((int13.i1.x - int13.i2.x) ** 2 + (int13.i1.y - int13.i2.y) ** 2);
const chordLength23 = Math.sqrt((int23.i1.x - int23.i2.x) ** 2 + (int23.i1.y - int23.i2.y) ** 2);

console.log(`弦の長さ: 円1-円2 = ${chordLength12.toFixed(1)}, 円1-円3 = ${chordLength13.toFixed(1)}, 円2-円3 = ${chordLength23.toFixed(1)}`);
console.log(`理論値 R√3 = ${(R * Math.sqrt(3)).toFixed(1)}\n`);

// 遠い交点から、弦の長さの1/4だけ重心方向に戻った位置
const dir12 = { x: centroid.x - far12.x, y: centroid.y - far12.y };
const intersect12 = moveInDirection(far12, dir12, chordLength12 / 4);
console.log(`intersect12: { x: ${Math.round(intersect12.x)}, y: ${Math.round(intersect12.y)} }`);

const dir13 = { x: centroid.x - far13.x, y: centroid.y - far13.y };
const intersect13 = moveInDirection(far13, dir13, chordLength13 / 4);
console.log(`intersect13: { x: ${Math.round(intersect13.x)}, y: ${Math.round(intersect13.y)} }`);

const dir23 = { x: centroid.x - far23.x, y: centroid.y - far23.y };
const intersect23 = moveInDirection(far23, dir23, chordLength23 / 4);
console.log(`intersect23: { x: ${Math.round(intersect23.x)}, y: ${Math.round(intersect23.y)} }`);

// only領域: 各円の中心から、他の2円の中心の平均点と反対方向に0.67R
const only1_direction = {
  x: circles[0].cx - (circles[1].cx + circles[2].cx) / 2,
  y: circles[0].cy - (circles[1].cy + circles[2].cy) / 2,
};
const only1 = moveInDirection({ x: circles[0].cx, y: circles[0].cy }, only1_direction, R * 0.67);
...
const only2 = moveInDirection({ x: circles[1].cx, y: circles[1].cy }, only2_direction, R * 0.67);
...
const only3 = moveInDirection({ x: circles[2].cx, y: circles[2].cy }, only3_direction, R * 0.67);
