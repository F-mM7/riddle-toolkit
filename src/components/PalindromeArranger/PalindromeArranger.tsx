...
import {
  solvePalindromeArrangement,
  solvePalindromeWithOneExclusion,
  SAMPLE_WORDS,
  type SolveResult,
  type SolveWithExclusionResult,
} from '../../utils/palindromeArranger/solver';
import styles from './PalindromeArranger.module.css';

type ResultState =
  | { mode: 'solve'; data: SolveResult }
  | { mode: 'exclusion'; data: SolveWithExclusionResult };

export function PalindromeArranger() {
  ...
  const handleSolve = () => {
    const solveResult = solvePalindromeArrangement(getWords());
    setResult({ mode: 'solve', data: solveResult });
    setClearCount(0);
  };

  const handleExclusionSolve = () => {
    const exclusionResult = solvePalindromeWithOneExclusion(getWords());
              ...
              <div className={styles.solutions}>
                {result.data.solutions.map((solution, idx) => (
                  <div key={idx} className={styles.solutionItem}>
                    ...
                    <p className={styles.palindromeString}>
                      {solution.palindrome}
                    </p>
